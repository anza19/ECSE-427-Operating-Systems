...
int my_system_f_read(char *user_command, char *my_fifo);
...
int main(int argc, char *argv[])
{
	...
	while(1)
	{
		...
		if(length(command) > 1)
		{
			...
			my_system_f_read(command, my_fifo);
...
//this is an implementation of the UNIX system call system() using fork() but with additional support to support the reading
//end of a named pipe
int my_system_f_read(char *user_command, char *my_fifo)
{
	...
	if(pid_checker == 0)
	{
		...
		//we are now rewiring the stdin to the reading end of our fifo
		//this means that anything reading from stdin will instead read from our fifo.
